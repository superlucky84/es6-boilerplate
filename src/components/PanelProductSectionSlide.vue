...
import {updateElement, currentAddingTargetId} from '@/store/types';

export default {
  name: 'PanelProductSectionSlide',
  props: {
    id: {
      type: String,
      required: true
  ...
  computed: {
    ...
    productCountOptions() {
      const maxProductCount = this.attr.columnCount === 3 ? 22 : 8;

      return _.range(this.attr.columnCount, maxProductCount, this.attr.columnCount);
    },
    sectionList() {
